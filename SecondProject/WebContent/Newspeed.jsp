...
<body>
	...
	<div class="container">
		...
		<div style="display: table; width: 96%;">
			<h3
				style="margin-left: 35px; font-weight: 700; float: left; margin-bottom: 0; margin-top: 25px;">여행후기</h3>


			<button class="btn btn-outline-black"
				style="float: right; height: 40px; margin-top: 25px; padding-top: 8px"
				onclick="gocal()">글쓰기</button>

		</div>
		<hr
			style="width: 1037px; border-top: 2px solid #eee; margin-top: 10px;">
		<div
			...
			<div class="diary">
				<a
					href="DiaryServlet?command=journalDetail&seq=<%=journallist.get(i).getSeq()%>">
					<div class="Dimage">
						<img alt="" onerror="this.src='img/img_is_not.png'"
							src="<%=journallist.get(i).getFisrt_Img() %>"
							style="width: 100%; height: 100%;">
					</div>
					<p class="diary-title"><%=journallist.get(i).getTitle() %></p>
				</a>
				<div class="diary-textbox">
					<span class="diary-id"><%=journallist.get(i).getId() %> 님</span> <span
				...
				<div class="diary-heartbox">
					<span style="text-align: right; color: #888; font-size: 14px;">
						<span class="diary-heart"></span> <%=journallist.get(i).getLike_cnt() %>
					</span>
				</div>
			</div>
			<%
